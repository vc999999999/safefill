...
5. 员工回传 `.yintian` 后，HR 只需指出回执位置和 Excel 输出位置。Agent 运行 `collect`，返回 `.xlsx` 及存在时的同名附件目录，并报告成功、排除和错误数量；不要把单元格明文贴到聊天中，除非 HR 明确要求查看。`collect` 返回的 `excluded` 大于 0 时，运行 `status` 或 `report` 查看逐人状态与原因，并用自然语言向 HR 概述被排除者与原因（仍不得贴出单元格明文）。大批量收件与 OCR 复核可能数分钟无输出，属正常现象。
...
- 开放任务的人工裁定（review/decide）经本地密钥解锁，仍需交互终端与逐项人工看图确认。
