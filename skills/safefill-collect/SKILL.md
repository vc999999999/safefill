...
description: 为 HR 或行政组织多人私密信息收集：生成 SafeFill 模板、管理加密提交、跟踪复核并交接本地 Excel 汇总。适用于群内收集身份资料；普通问卷和共享明细表不适用。
metadata:
...
以本文件目录为 `SKILL_ROOT`，使用已安装依赖的 Python 绝对路径。员工收到模板或邀请要填写时，改用同级 `safefill-fill` Skill（填写者），不在本 Skill 内代填。
...
- Agent 只读取任务元数据、字段定义，以及授权范围内的姓名、工号和进度状态；不读取或经手任何提交字段值、解密明文、密码、私钥、凭据内容（含数据库认证摘要）、保险柜内容、原始附件或明文 Excel。凭据可按已授权的私人收件人作为附件发送，但不得打开内容。
