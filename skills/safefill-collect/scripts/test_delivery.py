    ...
    def test_deterministic_package_and_independent_filler(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with zipfile.ZipFile(one) as archive:
                ...
                self.assertEqual({name.split('/')[0] for name in names}, {'safefill-collect', 'safefill-fill'})
            ...
            shutil.move(extracted / 'safefill-fill', filler)
            collector = root / 'collector-independent'
            shutil.move(extracted / 'safefill-collect', collector)
