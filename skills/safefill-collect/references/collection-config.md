...
默认使用 `open`：生成一份 `REQUEST.yintian-request` 机器请求包，不需要名单或个人凭据。请求包只描述告知、字段、校验规则和收集方公钥，不含网页界面或员工值；姓名由员工 Agent 在对话中收集并写入加密载荷。
...
开放请求只保证回执内容对非收集方保密，身份为员工自报。员工更正时必须带本人上一次 `.yintian`，以沿用随机回执编号；否则会形成新记录，`collect` 会在 `duplicate_names` 中提示同名多行，由 HR 与本人核对。

HR 明确要求"只有预先指定员工可提交"或"提交必须绑定工号"时，才使用兼容的 `group` 模式；该模式需要 `employee_id,name` 名单和逐人凭据，且 `create` 命令要求 HR 本人在交互终端运行并保管一次性任务密码——这与默认"HR 不运行终端"的体验不同，选用前必须说明。
