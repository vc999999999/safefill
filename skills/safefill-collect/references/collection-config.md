...
生成一份 `REQUEST.yintian-request` 机器请求包，不需要名单或个人凭据。请求包只描述告知、字段、校验规则和收集方公钥，不含网页界面或员工值；姓名由员工 Agent 在对话中收集并写入加密载荷。
...
开放请求只保证回执内容对非收集方保密，身份为员工自报。员工更正时必须带本人上一次 `.yintian`，以沿用随机回执编号；否则会形成新记录，`collect` 会在 `duplicate_names` 中提示同名多行，由 HR 与本人核对。需要"只有预先指定员工可提交"的强身份约束时，应采用独立认证渠道，本协议不提供名单或凭据机制。
