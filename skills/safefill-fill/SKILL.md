---
name: safefill-fill
description: 员工收到 SafeFill `.yintian-form` 或个人邀请后，从自己的加密保险柜本地填写并生成 `.yintian` 提交。适用于个人私密填报；不处理 HR 汇总、普通文案或他人资料。
metadata:
...
用户实际要求创建、分发、催办、查看进度、复核或汇总收集任务时，改用同级 `safefill-collect` Skill，不在本 Skill 内实现这些能力。
